...
Code snippet \ref{code:pymongo} represents the full PyMongo script I wrote to load the data into the database.
...
with open("Data/TextAnnotations.csv") as file3:
   ...
   for row in reader3:
      emage.update_many({'_id': int(row['emage_id'])},
         ...
         'gene' : {'geneID' : row['accession'],'name' : row['name']}}}}, upsert=False)
\end{lstlisting}
\vspace*{\fill}
\newpage